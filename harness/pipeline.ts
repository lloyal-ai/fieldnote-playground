...
import { resource, createContext, spawn, createChannel } from "effection";
...
import type {
  AgentEvent, Ability, AgentRenderCtx, Agent, DefaultAgentPolicyOpts, GuardOverrides,
...
function createResearchPolicy(
  ...
  /** `defaults.guards` from harness config: this harness's scope for the abilities' gates. */
  guards?: GuardOverrides,
  /** Whether the run has sources to gather evidence from: the floor applies only then. */
  hasSources = true,
): DefaultAgentPolicy {
  ...
  const opts: DefaultAgentPolicyOpts = {
    ...
    recoveryBudget: preset.reportBudget,
    guardOverrides: guards,
    hooks: hasSources ? [EVIDENCE_FIRST] : [],
...
/**
 * The evidence floor a research agent keeps: two tool calls before its report is
 * accepted. A first report is refused once with this message, then stands. The
 * framework used to default this for every pool; it is this harness's decision.
 */
const EVIDENCE_FIRST: ToolLifecycleHooks = {
  onReturn: ({ agent }) =>
    agent.toolCallCount < 2 ? { type: "reject", message: "You must use tools before submitting results." } : undefined,
};
...
function createReconPolicy(recoverPrompt: Prompt, guards?: GuardOverrides): ReconPolicy {
  return new ReconPolicy({
    // Recon runs every ability's tools, so it takes the harness's gate scope and evidence floor like research does.
    guardOverrides: guards,
...
/** Flat-mode fan-out: every task forks from the spine, each agent carrying
 *  its ability's preamble and its siblings' descriptions. How many run at once
 *  is the pool's: it seats what the context and its sequences can hold and
 *  admits the rest in waves, so a plan wider than the budget loses no task in
 *  silence. Each spec carries the task's key, which the fold reads off
 *  `agent:spawn` and the pool's `byKey` answers to. */
function researchSpecs(args: {
  tasks: ResearchTask[];
  abilityForTask: (task: ResearchTask) => Ability | undefined;
  maxTurns: number;
  date: string;
}): SpawnSpec[] {
  return args.tasks.map((task, i) => {
...
function assembleFindings(
  outcomes: readonly { result?: string | null }[],   // the pool's, one per task in spawn order, across heals
  tasks: ResearchTask[],
): string {
  return outcomes
    .map((a, i) => {
...
function* runPassthrough(
  ...
  yield* waitUntilSettled(trunk.prefill(userTurnTokens));
  ...
  for (;;) {
    ...
    if (isStop) break;
    yield* waitUntilSettled(trunk.commit(token));
    tokenCount++;
    pieces.push(text);
  }
  ...
  const { coverage, tokens, toolCalls } = yield* withSpine<{
    ...
    function* (reconSpine) {
      ...
      const probe = yield* agentPool({
        ...
        pruneOnReturn: true,
        policy: createReconPolicy(reconRecover, runner.config().defaults.guards),
        enableThinking: true,
...
export function* runResearchPlan(
  ...
  const {
    ...
    function* (querySpine) {
      ...
      const research = yield* agentPool({
        ...
        pruneOnReturn: true,
        policy: createResearchPolicy(opts.effort, opts.reasoningMode, opts.isAsk, recoveryPrompt, runner.config().defaults.guards, abilities.length > 0),
        scorer: primaryScorer,
        enableThinking: true,
        orchestrate:
          opts.reasoningMode === "flat"
            ? parallel(researchSpecs({ tasks, abilityForTask, maxTurns: opts.maxTurns, date: currentDate }))
      ...
      if (tasks.length === 1) {
        return {
          answer: research.outcomes[0]?.result?.trim() ?? "",
          totalTokens: research.totalTokens,
          totalToolCalls: research.totalToolCalls,
          synthTokens: 0,
        };
      ...
      if (research.outcomes.every((o) => !o.result?.trim())) {
        return {
          answer:
      ...
      const findings =
        opts.reasoningMode === "flat"
          ? assembleFindings(research.outcomes, tasks)
          : undefined;
      const synthCtx = {
        query,
        findings,
  ...
  if (answer) {
    if (opts.userSidePending) {
      ...
      yield* waitUntilSettled(session.prefillAssistant(answer));
    } else {
