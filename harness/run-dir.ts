 ...
 * `abort()` (nothing more written) — called by the harness the moment it
 * abandons a run, in the same order as the `start()` that may follow, never
 * read off the wire, which the sink sees behind that handler. A `ui:error`
 * is a toast and never touches it.
 *
...
export class RunDirSink {
   ...
   *  the docId (`outputDir/<docId>`), and it already exists: the harness
   *  creates it, exclusively, when it mints the identity, so the directory is
   *  the reservation and no two sessions can name one document. This writes
   *  into that reservation and creates nothing; a missing directory is a
   *  caller that skipped the mint, and says so. */
  start(opts: { dir: string; query: string; mode: 'flat' | 'deep'; attachments?: readonly string[] }): string {
    const dir = path.resolve(opts.dir);
    if (!fs.existsSync(dir)) {
      throw new Error(`RunDirSink.start: ${dir} is not reserved — a document's directory is created when its identity is minted`);
    }
  ...
  /** The run was stopped short. Annexures already written stay (the harness
   *  removes a stillborn dir itself); nothing more is written for this run.
   *  Called by the harness at the moment it abandons the run — not read off
   *  the wire: the sink sees the wire behind the handler, and an abort read
   *  there would end whichever run the handler had started since. */
  abort(): void {
    this.run = null;
  }
