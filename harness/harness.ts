...
import { spawn, each, call, ensure } from "effection";
...
import {
  reserveDocId as reserveDocDir,
  releaseUnfinished as releaseUnfinishedDir,
  runDirMode,
} from "./reservation.js";
...
export function* harness(
  ...
  yield* spawn(function* () {
    for (const ev of yield* each(agentEvents)) {
      ...
      if (ev.type === "complete") {
        ...
        // this one, and `run:aborted` must not be announced for it. The
        // report is on disk: the document is settled, no longer a reservation.
        if (run.docId !== null) reserved.delete(run.docId);
        run.task = null;
        run.docId = null;
        run.paused = false;
        run.woundDown = false;
  ...
  /** The folder IS the docId, reserved when the identity was minted. Whether
   *  the run threads beside a settled report or writes the document's own is
   *  `runDirMode`'s rule (`reservation.ts`): a new document never probes the
   *  disk, so no collision can turn it into a follow-up. */
  function startRunDir(
    ...
    const settled = warm && fs.existsSync(path.join(dir, "report.md"));
    if (runDirMode(warm, settled) === "thread") runDirSink.startThread({ dir, query, mode, attachments });
    else runDirSink.start({ dir, query, mode, attachments });
  }
  ...
  if (oneShot) {
    ...
    const oneShotDocId = reserveDocId();
    events.send({
      type: "query",
      docId: oneShotDocId,
  ...
  /** The unfinished reservations this connection owns: a document's directory
   *  is created — exclusively — when its identity is minted, and it stays a
   *  reservation until `report.md` settles it or it is released. Its own
   *  state, deliberately: `run.docId` is cleared by `startRun`'s finally the
   *  moment a run ends (before any teardown runs), `pendingPlan` by every
   *  abort, and a halt inside a handler visits no catch. Whatever is still
   *  here at teardown is released, so a disconnect or quit leaves no
   *  stillborn directory behind. */
  const reserved = new Set<DocId>();
  yield* ensure(() => {
    for (const id of [...reserved]) releaseUnfinished(id);
  });

  const libraryDir = (): string =>
    runner.config().sources.outputDir ?? process.cwd();

  /** Reserve a fresh identity (`reservation.ts` does the minting and the
   *  exclusive create) and own it until it settles or is released. */
  function reserveDocId(): DocId {
    const id = reserveDocDir(libraryDir());
    reserved.add(id);
    return id;
  }

  /** Give back a document that never settled, and forget the reservation. */
  function releaseUnfinished(docId: DocId): void {
    releaseUnfinishedDir(libraryDir(), docId);
    reserved.delete(docId);
  }
  ...
  function* abortRun({ keep = null }: { keep?: DocId | null } = {}): Operation<void> {
    ...
    const parkedDocId = pendingPlan?.docId ?? null;
    if (hadRun) yield* haltRun();
    // The sink stops writing for the abandoned run NOW, in this handler's
    // order, so the run this handler goes on to start is not the one an
    // abort read off the wire would have ended.
    runDirSink.abort();
    run.paused = false;
    run.woundDown = false;
    run.docId = null;
    // Whatever was abandoned and never settled goes — the dying run's
    // document and a parked plan's alike — EXCEPT the document the incoming
    // submit continues (`keep`): an ask on a parked document is a handoff,
    // not an abandonment, and its reservation is where the ask's first report
    // lands. And the harness's own memory of the id: knownDocs exists so
    // open_doc can activate the RUNNING doc; after an abort the doc is neither
    // running nor (if stillborn) on disk — a stale entry rendered an empty
    // ghost shell where the honest "no longer there" toast belongs. A standing
    // doc reopens via its report.md regardless.
    for (const id of new Set([dyingDocId, parkedDocId])) {
      if (id === null || id === keep) continue;
      releaseUnfinished(id);
      knownDocs.delete(id);
    }
  ...
  function* commitOpenedReport(): Operation<void> {
    ...
    openedReport = null;
    yield* waitUntilSettled(session.commitTurn(title, thread));
  }
    ...
    try {
      ...
      if (result.type === "research_plan") {
        ...
        if (spec.clarify === "commit") {
          yield* waitUntilSettled(session.commitTurn(spec.query, msg));
        } else if (spec.clarify === "prefill") {
          yield* waitUntilSettled(session.prefillAssistant(msg));
        }
      } else {
        pendingPlan = null;
        yield* reindexCorpus();
      ...
      // release what it never settled, then say why.
      pendingPlan = null;
      runDirSink.abort();
      releaseUnfinished(spec.docId);
      yield* agentEvents.send({ type: "run:aborted" });
  ...
  function* runAcceptedPlan(args: {
    /** The document this run writes — released if the run dies before it settles. */
    docId: DocId;
    query: string;
    plan: PlanResult;
    mode: "flat" | "deep";
  ...
  const handle: {
    ...
    *library_search(cmd) {
      ...
      let scores: number[];
      try {
        scores = yield* waitUntilSettled(reranker.scoreBatch(query, texts));
      } catch (err) {
        yield* agentEvents.send({
          type: "ui:error",
    ...
    *submit_query(cmd) {
      ...
      // active document. Anything else births a new identity — RESERVED
      // before anything is abandoned, so a mint that cannot reserve refuses
      // this submit with nothing changed, the run in flight included. The
      // attachments were checked above for the same reason: a rejected submit
      // is a toast, never an abort.
      ...
      if (warm) {
        ...
        try {
          ...
          yield* agentEvents.send({
            type: "ui:error",
            message: `Couldn't start a new document: ${errorMessage(err)}`,
          });
          return "continue";
        }
      }
      ...
      // The document this submit continues keeps its reservation: an ask on a
      // parked document is a handoff, and its first report lands there.
      ...
      if (prepared && prepared.bitmaps.length > 0) {
        ...
        yield* waitUntilSettled(session.prefillUserMultimodal(
          query,
      ...
      if (cmd.skipPlanner) {
        ...
        startRunDir(docId, query, mode, attachments, warm);
        yield* startRun(() =>
          runAcceptedPlan({ docId, query, plan, mode, wallStartMs, abilityFilter, isAsk: true, userSidePending, staged }),
        );
        return "continue";
      }
    ...
    *submit_clarification(cmd) {
      ...
      yield* waitUntilSettled(session.prefillUser(cmd.answer));
  ...
  for (const cmd of yield* each(commands)) {
    ...
    try {
      ...
      yield* abortRun();
      // A submit that threw between minting and starting left a reservation
      // nothing else names; after the abort every one still held is unfinished.
      for (const id of [...reserved]) releaseUnfinished(id);
      yield* agentEvents.send({ type: "ui:error", message: errorMessage(err) });
    }
