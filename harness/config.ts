...
function validateDefaults(raw: Partial<ConfigDefaults> | undefined): void {
  ...
  if (raw.guards !== undefined && !isGuardOverrides(raw.guards)) {
    throw new Error("defaults.guards must map gate names to false or { scope: lineage | cohort }");
  }
}
...
export function loadConfig(
  ...
  const localGuards = isGuardOverrides(ld?.guards) ? ld!.guards : undefined;
  ...
  // Absent means the harness contributes nothing and the framework's default
  // (lineage) applies to every gate — one default, owned by the framework, so
  // SHIPPED_DEFAULTS carries no entry.
  const guards = localGuards ?? yml.defaults?.guards;
