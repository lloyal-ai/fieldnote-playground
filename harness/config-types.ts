...
import type { GuardOverrides } from "@lloyal-labs/lloyal-agents";
...
export interface ConfigDefaults {
  ...
  maxTurns: number;
  /** This harness's overrides of the abilities' declared gates, by gate name:
   *  `false` switches one off, `{ scope: 'cohort' }` widens it from the agent's
   *  own lineage to the whole cohort. Research commits its two web gates
   *  cohort-wide in harness.yml, since sibling agents feed one synthesis.
   *  Absent = the framework's default (lineage) for every gate. */
