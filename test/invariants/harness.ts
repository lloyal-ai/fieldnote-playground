...
export interface HarnessSpec {
   ...
   *  `eventCount` is how many events the wire has carried so far. A halted
   *  run returns normally with `halted: true`. */
  controls?: (c: { halt: () => Promise<void>; eventCount: () => number }) => void;
}
...
export async function runHarness(spec: HarnessSpec = {}): Promise<HarnessRun> {
  ...
  try {
    await Promise.race([
      ...
      // scenario that asked for the halt, that is the run's normal end.
  ...
  return { events, trace: trace.events, outputDir, shutdownTraceIndex, halted };
}
