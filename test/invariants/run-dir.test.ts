...
test("start() writes into a reserved directory and creates none: a missing one is a caller that skipped the mint", () => {
  const lib = fs.mkdtempSync(path.join(os.tmpdir(), "run-dir-"));
  const missing = path.join(lib, "2026-01-01T00-00-00-000-0f0f0f0f-0000-4000-8000-000000000000");
  assert.throws(() => new RunDirSink().start({ dir: missing, query: "Q?", mode: "flat" }), /not reserved/);
  assert.equal(fs.existsSync(missing), false, "start() must not create the directory itself");
  const reserved = path.join(lib, "2026-01-01T00-00-00-001-0f0f0f0f-0000-4000-8000-000000000001");
  fs.mkdirSync(reserved);
  const sink = new RunDirSink();
  sink.start({ dir: reserved, query: "Q?", mode: "flat" });
  ...
  assert.ok(fs.existsSync(path.join(reserved, "report.md")), "the report lands in the reservation");
});

test("roots a tool result admitted ride the meta line beside the query's own, once each", () => {
  const lib = fs.mkdtempSync(path.join(os.tmpdir(), "run-dir-"));
  const dir = path.join(lib, "2026-01-01T00-00-00-000");
  fs.mkdirSync(dir);   // the reservation the harness makes when it mints the identity
